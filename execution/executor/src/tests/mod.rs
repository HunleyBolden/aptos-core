...
use aptos_types::{
    ...
    test_helpers::transaction_test_helpers::{block, BLOCK_EXECUTOR_ONCHAIN_CONFIG},
    transaction::{
...
fn execute_and_commit_block(
    ...
    let output = executor
        .execute_block(
            (id, block(vec![txn], BLOCK_EXECUTOR_ONCHAIN_CONFIG)).into(),
            parent_block_id,
...
fn test_executor_status() {
    ...
    let output = executor
        .execute_block(
            (
                block_id,
                block(vec![txn0, txn1, txn2], BLOCK_EXECUTOR_ONCHAIN_CONFIG),
            )
                .into(),
            parent_block_id,
            BLOCK_EXECUTOR_ONCHAIN_CONFIG,
...
fn test_executor_one_block() {
    ...
    let output = executor
        .execute_block(
            (block_id, block(txns, BLOCK_EXECUTOR_ONCHAIN_CONFIG)).into(),
            parent_block_id,
            BLOCK_EXECUTOR_ONCHAIN_CONFIG,
        )
        .unwrap();
...
fn test_executor_two_blocks_with_failed_txns() {
    ...
    let _output1 = executor
        .execute_block(
            (block1_id, block(block1_txns, BLOCK_EXECUTOR_ONCHAIN_CONFIG)).into(),
            parent_block_id,
            BLOCK_EXECUTOR_ONCHAIN_CONFIG,
        )
        .unwrap();
    let output2 = executor
        .execute_block(
            (block2_id, block(block2_txns, BLOCK_EXECUTOR_ONCHAIN_CONFIG)).into(),
            block1_id,
            BLOCK_EXECUTOR_ONCHAIN_CONFIG,
        )
        .unwrap();
...
fn test_executor_execute_same_block_multiple_times() {
    ...
    for _i in 0..100 {
        let output = executor
            .execute_block(
                (block_id, block(txns.clone(), BLOCK_EXECUTOR_ONCHAIN_CONFIG)).into(),
                parent_block_id,
                BLOCK_EXECUTOR_ONCHAIN_CONFIG,
            )
            .unwrap();
...
fn create_transaction_chunks(
    ...
    if BLOCK_EXECUTOR_ONCHAIN_CONFIG
        .block_gas_limit_type
        .block_gas_limit()
        .is_none()
    {
    ...
    let ledger_version =
        ledger_version_from_block_size(txns.len(), BLOCK_EXECUTOR_ONCHAIN_CONFIG) as u64;
...
fn test_noop_block_after_reconfiguration() {
    ...
    let second_block = TestBlock::new(10, 10, gen_block_id(2), BLOCK_EXECUTOR_ONCHAIN_CONFIG);
    let output2 = executor
        .execute_block(
...
fn test_reconfig_suffix_empty_blocks() {
    ...
    let block_a = TestBlock::new(10000, 1, gen_block_id(1), BLOCK_EXECUTOR_ONCHAIN_CONFIG);
    ...
    let block_c = TestBlock::new(1, 1, gen_block_id(3), BLOCK_EXECUTOR_ONCHAIN_CONFIG);
    let block_d = TestBlock::new(1, 1, gen_block_id(4), BLOCK_EXECUTOR_ONCHAIN_CONFIG);
    block_b
        .txns
...
proptest! {
    ...
    fn test_reconfiguration_with_retry_transaction_status(
        (num_user_txns, reconfig_txn_index) in (10..100u64).prop_flat_map(|num_user_txns| {
            ...
            let block_id = gen_block_id(1);
            let mut block = TestBlock::new(num_user_txns, 10, block_id, BLOCK_EXECUTOR_ONCHAIN_CONFIG);
            ...
            let output = executor.execute_block(
                (block_id, block.txns.clone()).into(), parent_block_id, BLOCK_EXECUTOR_ONCHAIN_CONFIG
            ).unwrap();
            ...
            let retry_output = executor.execute_block(
                (retry_block_id, block.txns.iter().skip(reconfig_txn_index as usize + 1).cloned().collect::<Vec<SignatureVerifiedTransaction>>()).into(), parent_block_id, BLOCK_EXECUTOR_ONCHAIN_CONFIG
            ).unwrap();
            prop_assert!(retry_output.compute_status().iter().all(|s| matches!(*s, TransactionStatus::Keep(_))));

            let ledger_version = ledger_version_from_block_size(num_txns as usize, BLOCK_EXECUTOR_ONCHAIN_CONFIG) as u64;
    ...
    fn test_executor_restart(a_size in 1..30u64, b_size in 1..30u64, amount in any::<u32>()) {
        ...
        let block_a = TestBlock::new(a_size, amount, gen_block_id(1), BLOCK_EXECUTOR_ONCHAIN_CONFIG);
        let block_b = TestBlock::new(b_size, amount, gen_block_id(2), BLOCK_EXECUTOR_ONCHAIN_CONFIG);
        ...
        // First execute and commit one block, then destroy executor.
        {
            ...
            let output_a = executor.execute_block(
                (block_a.id, block_a.txns.clone()).into(), parent_block_id, BLOCK_EXECUTOR_ONCHAIN_CONFIG
            ).unwrap();
            root_hash = output_a.root_hash();
            let ledger_info = gen_ledger_info(ledger_version_from_block_size(block_a.txns.len(), BLOCK_EXECUTOR_ONCHAIN_CONFIG) as u64, root_hash, block_a.id, 1);
        ...
        // Now we construct a new executor and run one more block.
        {
            ...
            let output_b = executor.execute_block((block_b.id, block_b.txns.clone()).into(), parent_block_id, BLOCK_EXECUTOR_ONCHAIN_CONFIG).unwrap();
            root_hash = output_b.root_hash();
            let ledger_info = gen_ledger_info(
                (ledger_version_from_block_size(block_a.txns.len(), BLOCK_EXECUTOR_ONCHAIN_CONFIG) + ledger_version_from_block_size(block_b.txns.len(), BLOCK_EXECUTOR_ONCHAIN_CONFIG)) as u64,
                root_hash,
                block_b.id,
                2,
            );
        ...
        let expected_root_hash = run_transactions_naive({
            ...
            if BLOCK_EXECUTOR_ONCHAIN_CONFIG.block_gas_limit_type.block_gas_limit().is_some() {
