...
use aptos_crypto::{
    ...
    p256_ecdsa,
    p256_ecdsa::P256_PUBLIC_KEY_LENGTH,
    secp256k1_ecdsa,
};
use aptos_types::{
...
impl VerifyInput for WebAuthnSignature {
    fn verify(&self) -> anyhow::Result<()> {
        ...
        // Currently only takes P256Ecdsa. If other signature schemes are introduced, modify this to accommodate them
        if public_key_len != P256_PUBLIC_KEY_LENGTH {
            bail!(
                "The public key provided is an invalid number of bytes, should be {} bytes but found {}. Note WebAuthn signatures only support P256Ecdsa at this time.",
                secp256k1_ecdsa::PUBLIC_KEY_LENGTH, public_key_len
            )
        } else {
            // TODO: Check if they match / parse correctly?
...
impl TryFrom<PublicKey> for AnyPublicKey {
    ...
    fn try_from(public_key: PublicKey) -> Result<Self, Self::Error> {
        Ok(match public_key {
            ...
            PublicKey::P256Ecdsa(p) => AnyPublicKey::p256_ecdsa(p.inner().try_into()?),
        })
    }
}
...
impl From<AnyPublicKey> for PublicKey {
    fn from(key: AnyPublicKey) -> Self {
        match key {
            ...
            AnyPublicKey::P256Ecdsa { public_key } => {
                PublicKey::P256Ecdsa(public_key.to_bytes().to_vec().into())
            },
        }
    }
}
...
impl VerifyInput for SingleKeySignature {
    fn verify(&self) -> anyhow::Result<()> {
        match (&self.public_key, &self.signature) {
            ...
            (PublicKey::P256Ecdsa(p), Signature::WebAuthn(s)) => WebAuthnSignature {
                public_key: p.clone(),
                signature: s.clone(),
            }
            .verify(),
...
impl TryFrom<SingleKeySignature> for AccountAuthenticator {
    ...
    fn try_from(value: SingleKeySignature) -> Result<Self, Self::Error> {
        let key = match value.public_key {
            ...
            PublicKey::P256Ecdsa(p) => {
                let key = p
                    .inner()
                    .try_into()
                    .context("Failed to parse given public_key bytes as P256EcdsaPublicKey")?;
                AnyPublicKey::p256_ecdsa(key)
            },
        };
        ...
        for public_key in value.public_keys {
            let key =
                match public_key {
                    ...
                    PublicKey::P256Ecdsa(p) => {
                        let key = p.inner().try_into().context(
                            "Failed to parse given public_key bytes as P256EcdsaPublicKey",
                        )?;
                        AnyPublicKey::p256_ecdsa(key)
                    },
                };
